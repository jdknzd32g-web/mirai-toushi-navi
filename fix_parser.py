...
code = open("tools/auto_blog_generator.py").read()

new_content = """def generate_seo_description(raw_text, title):
...
code = re.sub(r"def beautify_text\(text\):.*?def update_sitemap\(slug, title, image_name\):", new_content, code, flags=re.DOTALL)
