 ...
 *
 * 使い方:
 * npx tsx src/x-post-with-image.ts --file ~/Desktop/テキスト.txt --image ~/Desktop/画像.jpg
 * npx tsx src/x-post-with-image.ts --slot morning --root "/Users/satoshioka/YouTube project" --image ~/Desktop/画像.jpg
 * npx tsx src/x-post-with-image.ts --file ~/Desktop/テキスト.txt --image ~/Desktop/画像.jpg --dry-run
 */
...
async function main() {
  const { slot, root, dry, file: fileArg, force, image: imageArg } = parseArgs();
  ...
  if (dry) {
    console.log('--- dry-run (x-post-with-image) ---');
  ...
  if (imageArg) {
    ...
    try {
      ...
      const upload = await rw.v1.uploadMedia(imageBuffer, {
        mimeType: getMediaType(imageArg)
      });
      mediaId = upload;
      console.log(`✓ 画像をアップロード: ${imageArg} (mediaId: ${mediaId})`);
    } catch (err) {
