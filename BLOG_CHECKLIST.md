...
- [ ] 「投資の世界」「プロの投資家」などのNGワードが含まれていないか
- [ ] 本文、リード文、装飾ボックス、CTA、プロフィールが一つのセンテンスごとに表示改行されているか
- [ ] 同じ話題は`<br>`、話題が変わる場所は`<p>`で分けられているか
- [ ] センテンスごとに`<p>`を分け、段落間の余白が過剰になっていないか
- [ ] 本文中の強調が`<strong class="highlight">`で太字＋黄色マーカーになっているか
- [ ] 文全体ではなく、重要な短いフレーズだけが強調されているか
